...
interface PagBankPaymentModalProps {
  isOpen: boolean;
  onClose: () => void;
  amount: number;
  onSuccess: () => void;
...
export function PagBankPaymentModal({ isOpen, onClose, amount, onSuccess }: PagBankPaymentModalProps) {
  const { user } = useAuth();
  const { t } = useLanguage();
  ...
  async function createPayment() {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-pagbank-payment`, {
        method: 'POST',
        headers: {
  ...
  function startPaymentPolling(orderId: string) {
    const pollInterval = setInterval(async () => {
      try {
        ...
        const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/check-pagbank-payment`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${session.access_token}`,
            'Content-Type': 'application/json',
        ...
        if (result.success && result.payment && result.payment.status === 'approved') {
          ...
          onSuccess();
          onClose();
        } else if (result.payment && ['REFUNDED', 'EXPIRED', 'CANCELED', 'rejected'].includes(result.payment.status)) {
          clearInterval(pollInterval);
  ...
  async function checkPaymentStatus() {
    ...
    try {
      ...
      if (result.success && result.payment) {
        ...
        if (['REFUNDED', 'EXPIRED', 'CANCELED', 'rejected'].includes(result.payment.status)) {
          setError('Pagamento expirado ou cancelado.');
          return;
        }
      ...
      <div className="relative top-10 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white dark:bg-gray-800 max-h-[90vh] overflow-y-auto">
        ...
        {!paymentData ? (
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 dark:bg-gray-900/40 border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Obrigatorio para gerar cobrancas via PagBank.
                </p>
              </div>
            ...
            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <span className="text-green-700 dark:text-green-400 text-sm">Valor:</span>
                  <span className="ml-2 font-bold text-green-900 dark:text-green-200 text-lg">
                    R$ {(amount * 5.5).toFixed(2)}
                  </span>
                </div>
                <div>
                  <span className="text-green-700 dark:text-green-400 text-sm">Creditos:</span>
                  <span className="ml-2 font-bold text-green-600 dark:text-green-400">
                    ${amount.toFixed(2)}
                  </span>
                </div>
                <div>
                  <span className="text-green-700 dark:text-green-400 text-sm">Metodo:</span>
                  <span className="ml-2 font-medium text-green-900 dark:text-green-200">
          ...
          <div className="space-y-6">
                ...
                {timeLeft > 0 && (
                  <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
                    <div className="flex items-center justify-center space-x-2">
                      ...
                      <span className="text-sm text-green-600 dark:text-green-400">restantes</span>
                    </div>
                  </div>
                )}
            ...
            {/* PIX QR Code as image URL */}
            {paymentMethod === 'pix' && !paymentData.qr_code_base64 && paymentData.qr_code_image && (
              <div className="space-y-4">
                {timeLeft > 0 && (
                  <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
                    <div className="flex items-center justify-center space-x-2">
