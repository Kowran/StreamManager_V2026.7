...
import { Plus, CreditCard as Edit, Trash2, Package, Search, Save, X, DollarSign, List, Upload, Star, Store, ShoppingCart } from 'lucide-react';
...
export function SellerStore() {
  ...
  async function removeInventoryItem(index: number) {
    ...
    if (item.id) {
      const confirmMessage = `Tem certeza que deseja excluir esta conta do estoque?\n\nEmail: ${item.email}\n\nEsta ação não pode ser desfeita.`;
      ...
      try {
        ...
        if (error) {
          ...
          alert('Erro ao excluir conta do estoque. Tente novamente.');
          return;
        }

        alert('✅ Conta excluída do estoque com sucesso!');
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Apenas vendedores aprovados podem gerenciar sua loja.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Minha Loja</h2>
