...
interface EfiPaymentModalProps {
  isOpen: boolean;
  onClose: () => void;
  amount: number;
  onSuccess: () => void;
...
export function EfiPaymentModal({ isOpen, onClose, amount, onSuccess }: EfiPaymentModalProps) {
  const { user } = useAuth();
  const { t } = useLanguage();
  ...
  function validateForm(): boolean {
    ...
    if (cpfDigits.length !== 11 && cpfDigits.length !== 14) {
      setFormError('Por favor, informe um CPF (11 digitos) ou CNPJ (14 digitos) valido.');
      return false;
    }
    return true;
  }
  ...
  async function createPayment() {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-efi-payment`, {
        method: 'POST',
        headers: {
  ...
  function startPaymentPolling(orderId: string) {
    const pollInterval = setInterval(async () => {
      try {
        ...
        const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/check-efi-payment`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${session.access_token}`,
            'Content-Type': 'application/json',
        ...
        if (result.success && result.payment && result.payment.status === 'approved') {
          ...
          onSuccess();
          onClose();
        } else if (result.payment && ['REMOVIDA_PELO_USUARIO_RECEBEDOR', 'EXPIRED', 'CANCELED', 'rejected'].includes(result.payment.status)) {
          clearInterval(pollInterval);
  ...
  async function checkPaymentStatus() {
    ...
    try {
      ...
      if (result.success && result.payment) {
        ...
        if (['REMOVIDA_PELO_USUARIO_RECEBEDOR', 'EXPIRED', 'CANCELED', 'rejected'].includes(result.payment.status)) {
          setError('Pagamento expirado ou cancelado.');
          return;
        }
        ...
        {!paymentData ? (
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 dark:bg-gray-900/40 border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Obrigatorio para gerar cobrancas via EFI Bank.
                </p>
              </div>
            ...
            <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4">
              <h4 className="text-lg font-medium text-orange-800 dark:text-orange-300 mb-3">
                Resumo do Pagamento
              </h4>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <span className="text-orange-700 dark:text-orange-400 text-sm">Valor:</span>
                  <span className="ml-2 font-bold text-orange-900 dark:text-orange-200 text-lg">
                    R$ {(amount * 5.5).toFixed(2)}
                  </span>
                </div>
                <div>
                  <span className="text-orange-700 dark:text-orange-400 text-sm">Creditos:</span>
                  <span className="ml-2 font-bold text-green-600 dark:text-green-400">
                    ${amount.toFixed(2)}
                  </span>
                </div>
                <div>
                  <span className="text-orange-700 dark:text-orange-400 text-sm">Metodo:</span>
                  <span className="ml-2 font-medium text-orange-900 dark:text-orange-200">
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-orange-600 hover:bg-orange-700 disabled:bg-gray-400 text-white font-medium py-3 px-6 rounded-lg transition-colors flex items-center justify-center space-x-2"
            >
              {loading ? (
                <>
          ...
          <div className="space-y-6">
            ...
            {paymentMethod === 'boleto' && paymentData.invoice_url && (
              <div className="text-center space-y-4">
                <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4">
                  <FileText className="h-12 w-12 mx-auto mb-3 text-orange-600" />
                  <h4 className="text-lg font-medium text-orange-800 dark:text-orange-300 mb-2">
                    Boleto Gerado!
                  </h4>
            ...
            <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4">
              ...
              {paymentMethod === 'pix' ? (
                ...
                <ol className="text-xs text-orange-700 dark:text-orange-400 space-y-1 list-decimal list-inside">
                  ...
                  <li>O credito e adicionado apos o pagamento ser confirmado (1-3 dias uteis)</li>
                </ol>
              )}
            </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-600">
              <button
                ...
                disabled={checking}
                className="inline-flex items-center px-4 py-2 bg-orange-600 hover:bg-orange-700 disabled:bg-gray-400 text-white font-medium rounded-lg transition-colors shadow-sm"
              >
                {checking ? (
