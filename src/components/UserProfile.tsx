...
import { User, CreditCard as Edit, Save, X, Mail, Calendar, Globe, Shield, Eye, EyeOff, Check, AlertCircle, Camera, Upload, Store, Bell, BellOff } from 'lucide-react';
...
export function UserProfile({ onNavigate }: UserProfileProps = {}) {
  ...
  async function loadUserProfile() {
    ...
    try {
      ...
      setProfile(data);
      setHideExpiringBalloon(data.hide_expiring_balloon || false);
    } catch (error) {
              ...
              {hasStreamingAccess && (
                <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h4 className="text-sm font-medium text-gray-900 dark:text-white">
                        {t.language === 'pt' ? 'Balão de Contas Vencidas' : t.language === 'en' ? 'Expiring Accounts Balloon' : 'Globo de Cuentas Vencidas'}
                      </h4>
                      <p className="text-xs text-gray-600 dark:text-gray-400">
                        {t.language === 'pt'
                          ? 'Ative ou desative o balão flutuante de contas vencidas'
                          : t.language === 'en'
                          ? 'Enable or disable the floating expiring accounts balloon'
                          : 'Activa o desactiva el globo flotante de cuentas vencidas'}
                      </p>
                    </div>
                  </div>
                  <button
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="space-y-3">
              ...
              {profile.role === 'seller' && onNavigate && (
                ...
                >
                  <Store className="h-4 w-4" />
                  <span>{language === 'pt' ? 'Minha Loja' : language === 'en' ? 'My Store' : 'Mi Tienda'}</span>
                </button>
              )}
