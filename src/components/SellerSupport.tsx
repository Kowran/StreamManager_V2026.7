            ...
            {selectedTicket.status === 'resolved' && selectedTicket.resolution_type && (
              <div className={`mt-3 border rounded-lg p-3 ${
                ...
                <div className="flex items-center gap-2 mb-1">
                  <CheckCircle className={`h-4 w-4 ${selectedTicket.admin_resolved ? 'text-amber-600 dark:text-amber-400' : 'text-green-600 dark:text-green-400'}`} />
                  <p className={`text-xs font-semibold ${selectedTicket.admin_resolved ? 'text-amber-800 dark:text-amber-300' : 'text-green-800 dark:text-green-300'}`}>
                    {selectedTicket.admin_resolved
                ...
                {selectedTicket.admin_resolved && (
                  <div className="mb-1">
                    <span className={`text-xs font-medium ${selectedTicket.admin_resolved ? 'text-amber-800 dark:text-amber-300' : 'text-green-800 dark:text-green-300'}`}>
                      {selectedTicket.resolution_type === 'refund'
                        ? lbl('Decisão: Reembolso ao cliente', 'Decision: Refund to customer', 'Decisión: Reembolso al cliente')
                        : selectedTicket.resolution_type === 'replace_account'
                          ? lbl('Decisão: Substituição de produto', 'Decision: Product replacement', 'Decisión: Reemplazo de producto')
                          : lbl('Disputa encerrada', 'Dispute closed', 'Disputa cerrada')}
                    </span>
                  </div>
                )}
                {selectedTicket.resolution_notes && selectedTicket.admin_resolved && (
                  <div className={`text-xs mb-1 bg-white/60 dark:bg-gray-800/40 rounded p-2 ${selectedTicket.admin_resolved ? 'text-amber-700 dark:text-amber-400' : 'text-green-700 dark:text-green-300'}`}>
                    <span className="font-medium">{lbl('Notas do administrador:', 'Admin notes:', 'Notas del administrador:')} </span>
                ...
                {selectedTicket.admin_resolved && selectedTicket.resolved_at && (
                  <p className="text-xs text-amber-500 dark:text-amber-500 mt-1">
                    {lbl('Resolvido em', 'Resolved on', 'Resuelto el')}: {new Date(selectedTicket.resolved_at).toLocaleString(language === 'pt' ? 'pt-BR' : 'en-US')}
                  </p>
                )}
              </div>
            )}
