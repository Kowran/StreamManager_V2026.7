...
import { X, Save, CreditCard, AlertCircle, CheckCircle, Eye, EyeOff, Shield, Key } from 'lucide-react';
...
export function StripeConfigModal({ isOpen, onClose, onSave }: StripeConfigModalProps) {
  const { t } = useLanguage();
  ...
  async function loadConfig() {
    ...
    try {
      ...
      setError('Erro ao carregar configurações do Stripe');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function testStripeConnection() {
    ...
    try {
      ...
      if (result.success) {
        alert('✅ Conexão com o Stripe estabelecida com sucesso!');
      } else {
        throw new Error(result.error || 'Falha na conexão com o Stripe');
      }
    } catch (error) {
    ...
    try {
      ...
      if (config.test_mode) {
        if (!config.publishable_key.startsWith('pk_test_')) {
          console.warn('Warning: Publishable Key should start with pk_test_ in test mode');
        }
        if (!config.secret_key.startsWith('sk_test_')) {
          console.warn('Warning: Secret Key should start with sk_test_ in test mode');
        }
      } else {
        if (!config.publishable_key.startsWith('pk_live_')) {
          console.warn('Warning: Publishable Key should start with pk_live_ in production mode');
        }
        if (!config.secret_key.startsWith('sk_live_')) {
          console.warn('Warning: Secret Key should start with sk_live_ in production mode');
        }
      }
      ...
      const { error } = await supabase
        ...
        .upsert({
          ...
          description: 'Configurações do Stripe para pagamentos com cartão',
      ...
      <div className="relative top-10 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white dark:bg-gray-800 max-h-[90vh] overflow-y-auto">
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              O Stripe foi configurado com sucesso.
            </p>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Chave secreta usada no backend (NUNCA exponha no frontend)
                </p>
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Secret usado para verificar webhooks do Stripe
                </p>
              </div>
            </div>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
              ...
              <p className="mt-2 text-xs text-blue-700 dark:text-blue-400">
                Configure esta URL no painel do Stripe para receber notificações de pagamento
              </p>
            </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
              ...
              <ol className="text-xs text-gray-700 dark:text-gray-300 space-y-1 list-decimal list-inside">
                <li>Acesse o <a href="https://dashboard.stripe.com" target="_blank" className="text-blue-600 hover:underline">Dashboard do Stripe</a></li>
                <li>Vá para "Developers" → "API keys"</li>
                <li>Copie a "Publishable key" e "Secret key"</li>
                <li>Vá para "Developers" → "Webhooks"</li>
                <li>Crie um novo webhook com a URL fornecida acima</li>
                <li>Selecione os eventos: payment_intent.succeeded, payment_intent.payment_failed</li>
                <li>Copie o "Signing secret" do webhook</li>
                <li>Cole todas as chaves nos campos acima</li>
            ...
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
              ...
              <ul className="text-xs text-red-700 dark:text-red-400 space-y-1">
                <li>• NUNCA compartilhe sua Secret Key</li>
                <li>• Use sempre HTTPS em produção</li>
                <li>• Mantenha as chaves seguras e rotacione regularmente</li>
                <li>• Configure webhooks para confirmação automática</li>
                <li>• Monitore transações no dashboard do Stripe</li>
              </ul>
            </div>
            ...
            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs text-green-700 dark:text-green-400">
                <div>• Cartões de crédito/débito</div>
                <div>• Apple Pay / Google Pay</div>
                <div>• Pagamentos internacionais</div>
                <div>• Confirmação automática</div>
                <div>• Reembolsos</div>
                <div>• Proteção contra fraude</div>
              </div>
            </div>
